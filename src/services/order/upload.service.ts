...
// export const processOrderUpload = async (fileBuffer: Buffer) => {
...
//                 metadata: {
//                     platform: 'TEMU',
//                     fulfillmentMode: raw['Fulfillment mode'],
...
import { parseBufferFileToHeadersArray } from '@utils/file-utils';
import OrderBase from '@models/Order';
// Import other platform mappers
...
export const processOrderUpload = async (fileBuffer: Buffer) => {
    ...
    try {
        ...
        const headers = parseBufferFileToHeadersArray(fileBuffer);
        ...
        return {
            success: true,
            insertedCount: result.insertedCount,
            firstOrder: results[0] // For verification
        };
    } catch (error) {
        console.error('Upload processing failed:', error);
        throw error; // Let controller handle the response
